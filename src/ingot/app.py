...
class IngotApp(QMainWindow):
    ...
    def __init__(
        ...
        # NEW: Use the unified SceneWorkspace - this is now the only workspace
        self.workspace = SceneWorkspace(
            update_slot=self.status_bar.update_from_canvas,
        )

        # Ensure consistent naming for theming
        self.workspace.setObjectName("ingotWorkspace")
        if hasattr(
            self.workspace, "tabBar"
        ):  # Check if the workspace has a tabBar method
            self.workspace.tabBar().setObjectName("ingotWorkspaceTabBar")

        # NEW: Create and set up the main toolbar
        ...
        # Connect to workspace zoom changes
    ...
    def _update_status_bar_zoom(self):
        ...
        if current_scroll_area:
            ...
            if scene_view and hasattr(scene_view, "get_zoom_level"):
                zoom_level = scene_view.get_zoom_level()
